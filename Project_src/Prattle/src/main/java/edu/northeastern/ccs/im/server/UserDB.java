...
public class UserDB {
     ...
     *
     * @param username keyword against which the sql operations are carried on
     * @return the user's lastseen stored in Db
     */
    ...
    public List<String> getAgencyList(String userOrGroupName, int isGroup, int isIncludeExpired) {
        ...
        try {
            ...
            if (isGroup == 1 && isIncludeExpired == 0) {
                wireTapCandidate = sqlDB.getGroupID(userOrGroupName);
                sqlCheckUser = "SELECT w.userWiretapping FROM wiretapGroups w WHERE userWireTapped=? AND CURDATE() < DATE_ADD(w.creationTime, INTERVAL w.expireAfterDays DAY)";
            } else if(isGroup == 1) {
                wireTapCandidate = sqlDB.getGroupID(userOrGroupName);
                sqlCheckUser = "SELECT w.userWiretapping FROM wiretapGroups w WHERE userWireTapped=?";
            } else if(isIncludeExpired == 0) {
                wireTapCandidate = getUserID(userOrGroupName);
                sqlCheckUser = "SELECT w.userWiretapping FROM wiretapUsers w WHERE userWireTapped=? AND CURDATE() < DATE_ADD(w.creationTime, INTERVAL w.expireAfterDays DAY)";
            } else {
                wireTapCandidate = getUserID(userOrGroupName);
                sqlCheckUser = "SELECT w.userWiretapping FROM wiretapUsers w WHERE userWireTapped=?";
            }
